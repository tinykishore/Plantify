<footer class="hidden fixed bottom-0 w-full md:flex mx-auto gap-x-10 py-2 px-20 border-t border-t-zinc-200 z-50
justify-between sm:justify-between align-middle items-center text-sm text-zinc-600 shadow-sm bg-white
rounded-t">
    <div id="left-side" class="flex flex-row gap-2 gap-x-6 flex-wrap">
        <p class="select-none">&copy; 2023 Plantify, Inc.</p>
        <a href="/terms" class="hover:text-teal-500 hover:underline">Terms</a>
        <a href="/error" class="hover:text-teal-500 hover:underline">Sitemap</a>
        <a href="/error" class="hover:text-teal-500 hover:underline">Privacy</a>
        <a href="/error" class="hover:text-teal-500 hover:underline">Your Privacy Choices</a>
    </div>

    <div class="text-gray-900 font-bold flex-row flex gap-x-6">
        <a href="/error" class="hover:underline">English (US)</a>
        <a href="/error" class="hover:underline">Support and Resource</a>
    </div>

</footer>